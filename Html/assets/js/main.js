$(function () {
  ...
  if (page === "home") {
    var targets = { clients: 1200, savings: 75, years: 10 };
    $(".metric-value").each(function () {
      var $el = $(this);
      var key = $el.data("metric");
    ...
    function updateDeadlineCountdown() {
      // Set deadline date (July 31st of current year)
      var currentYear = new Date().getFullYear();
      var deadlineDate = new Date(currentYear, 6, 31); // Month 6 = July (0-indexed)
      var now = new Date();
      
      // If deadline has passed, set for next year
      if (now > deadlineDate) {
        deadlineDate = new Date(currentYear + 1, 6, 31);
      }
      ...
      $("#deadline-countdown").text(countdownText);
    }
  ...
  $("#mini-enquiry").on("submit", function (e) {
    e.preventDefault();
    alert("Mini enquiry submitted. Hook this form to your backend mail API.");
    this.reset();
  });
  ...
  $("#contact-form").on("submit", function (e) {
    ...
    $status.addClass("success").text("Submitted successfully. Configure server-side email sending to complete the flow.");
    this.reset();
  });
  ...
  $("#service-contact-form").on("submit", function (e) {
    ...
    $status.addClass("success").text("Submitted successfully. Our team will contact you within one business day.");
    // Don't reset the form, keep service name filled
    $("#service-contact-name").val("");
    $("#service-contact-email").val("");
    $("#service-contact-phone").val("");
    $("#service-contact-message").val("");
  });
